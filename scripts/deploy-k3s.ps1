...
param(
    ...
    [string]$PriceAnalyzerImage = "darkwing-priceanalyzer:local",
...
Write-Host "Building $PriceAnalyzerImage ..."
docker build -t $PriceAnalyzerImage -f "$Root/src/PriceAnalyzer/Dockerfile" $Root
Assert-LastExitCode "docker build ($PriceAnalyzerImage)"
...
switch ($Runtime) {
    "docker-desktop" {
        ...
        foreach ($image in @($ApiImage, $WorkerImage, $PricePickerImage, $PriceAnalyzerImage)) {
            $safe = ($image -replace "[:/]", "-")
    ...
    "k3d" {
        ...
        k3d image import $ApiImage $WorkerImage $PricePickerImage $PriceAnalyzerImage -c $K3dCluster
    ...
    "k3s" {
        ...
        docker save $PriceAnalyzerImage | sudo k3s ctr images import -
        Assert-LastExitCode "k3s ctr import ($PriceAnalyzerImage)"
    }
}
...
Write-Host "  kubectl -n darkwing logs -f deploy/priceanalyzer"
